 ...
 * create by NotCoder
 * date 2019/12/25
 * Describe: 可以触摸拖动的浮动按钮
 */

public class BaseAttachButton extends FloatingActionButton {
    ...
    public BaseAttachButton(Context context) {
        this(context, null);
    }

    public BaseAttachButton(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BaseAttachButton(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_UP://手指离开
                if(hasMove){
                    ...
                    //自动贴边
                    if (mLastRawX <= center /1.5) {
                        //向左贴边
                        BaseAttachButton.this.animate()
                                .setInterpolator(new BounceInterpolator())
                                .setDuration(500)
                                .x(0)
                                .start();
                    } else if (mLastRawX >= mRootMeasuredWidth - center / 1.5) {
                        //向右贴边
                        BaseAttachButton.this.animate()
                                .setInterpolator(new BounceInterpolator())
                                .setDuration(500)
